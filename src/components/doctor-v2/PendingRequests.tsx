...
export default function PendingRequests() {
  ...
  const { data, isLoading } = useQuery<{ items: Block[] }>({
  ...
  const items = (data?.items ?? []).filter((b) => b.estado === "pendiente")
  ...
  return (
    <Card className="rounded-2xl border border-border/60 bg-white/95 shadow-lg shadow-primary/10">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-secondary/70">Bloqueos en revisión</p>
      </CardHeader>
        ...
        {items.map((b) => (
          <div key={b.id} className="flex items-center justify-between rounded-xl border border-border/60 bg-secondary/5 px-3 py-2">
            <div className="flex flex-col">
              ...
              <span className="text-xs text-secondary/70">{b.inicio} - {b.fin} · {b.motivo}</span>
            </div>
